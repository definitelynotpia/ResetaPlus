...
class QrCodeDisplay extends StatefulWidget {
  ...
  const QrCodeDisplay({super.key, required this.prescriptionId});
...
class _QrCodeDisplayState extends State<QrCodeDisplay> {
  ...
  Future<void> fetchQrCode() async {
    try {
      String? filePath = await fetchQrCodeFilePath(
          widget.prescriptionId); // Fetch the file path
  ...
  Future<String?> fetchQrCodeFilePath(int prescriptionId) async {
    ...
    try {
      debugPrint(
          "Fetching QR code file path for prescription ID: $prescriptionId");
      ...
      if (result.rows.isNotEmpty) {
        ...
        return result.rows.first
            .colByName('qr_code_filepath')
            ?.toString(); // Change type casting as necessary
      } else {
